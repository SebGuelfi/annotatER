...
#' @param flankingRegion flanking region in bp to add to the region

annotatERJunction <- function(regions,annotatedJunction,flankingRegion=0)
{
    ##############################################################################
    ...
    if((grepl("chr",as.character(annotatedJunction$chr[1])) & grepl("chr",as.character(seqnames(regions))[1])) ||
       ((!grepl("chr",as.character(annotatedJunction$chr[1]))) & (!grepl("chr",as.character(seqnames(regions))[1]))))
    {
    ...
    }else if(grepl("chr",as.character(seqnames(regions))[1])){
    ...
    regionJuncOve <- findOverlaps(regions,juncti.GR,ignore.strand=FALSE,maxgap=flankingRegion)
    ...
    tmp.regions[regionJuncOve.df$queryHits,c(colnames( tmp.annotatedJunction)[c(1,6:11)],"pred_strand")] <- tmp.annotatedJunction[match(mcols(juncti.GR[regionJuncOve.df$subjectHits])[,"junID"],tmp.annotatedJunction$junID),c(colnames( tmp.annotatedJunction)[c(1,6:11)],"strand")]
    regionJuncOve.df <- as.data.frame(regionJuncOve)
    regionJuncOve.df <- cbind(regionJuncOve.df,tmp.annotatedJunction[match(mcols(juncti.GR[regionJuncOve.df$subjectHits])[,"junID"],tmp.annotatedJunction$junID),c("acceptor","donor")])
    regionJuncOve.df <- regionJuncOve.df[!regionJuncOve.df$queryHits%in%which(!is.na(tmp.regions$junID)),]
    ...
    tmp.regions[!is.na(tmp.regions$junER),"distance"] <- apply(abs(cbind(tmp.regions[!is.na(tmp.regions$junER),"start"] -
                                                                             tmp.regions[tmp.regions[!is.na(tmp.regions$junER),"junER"],"end"],
                                                                         tmp.regions[!is.na(tmp.regions$junER),"end"] -
                                                                             tmp.regions[tmp.regions[!is.na(tmp.regions$junER),"junER"],"start"],
                                                                         tmp.regions[!is.na(tmp.regions$junER),"start"] -
                                                                             tmp.regions[tmp.regions[!is.na(tmp.regions$junER),"junER"],"end"],
                                                                         tmp.regions[!is.na(tmp.regions$junER),"end"] -
                                                                             tmp.regions[tmp.regions[!is.na(tmp.regions$junER),"junER"],"start"])),1,min)
    ...
    message(paste(Sys.time(),"--",
                  paste0(round((table(tmp.regions$annotationType==1)["TRUE"]/nrow(tmp.regions))*100,digits = 2),"%"),"regions annotated with known genes"))
